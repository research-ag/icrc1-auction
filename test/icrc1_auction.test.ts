...
describe('ICRC1 Auction', () => {
  ...
  describe('canister installation', () => {
    ...
    test('should preserve info during upgrade', async () => {
      ...
      await auction.manageDarkOrderBooks(
        [
          [
            ...
            [
              await encryptOrders([
                { kind: 'ask', volume: 1000000, price: 42.5 },
                {
                  kind: 'bid',
                  volume: 12300,
                  price: 13.2,
      ...
      expect(
        (await auction.auction_query([ledger2Principal], { ...auctionQueryEmpty, bids: [true] })).bids,
      ).toHaveLength(1);
      expect(
        (await auction.auction_query([ledger2Principal], { ...auctionQueryEmpty, dark_order_books: [true] }))
          .dark_order_books,
      ).toHaveLength(1);
      ...
      expect(
        (
          await auction.auction_query([ledger2Principal], {
            ...auctionQueryEmpty,
            bids: [true],
          })
        ).bids,
  ...
  describe('orders', () => {
    test('should be able to manage orders via single query', async () => {
      ...
      let res2 = await auction.manageOrders(
        [{ all: [] }],
        [
    ...
    test('bids should affect metrics', async () => {
      ...
      expect(
        (
          await auction.auction_query([ledger1Principal], {
            ...auctionQueryEmpty,
            bids: [true],
          })
        ).bids,
  ...
  describe('dark order books', () => {
    ...
    test('should lock funds when dark order book set', async () => {
      ...
      let state = await auction.auction_query([quoteLedgerPrincipal, ledger1Principal, ledger2Principal], {
        ...auctionQueryEmpty,
        dark_order_books: [true],
        credits: [true],
      });
    ...
    test('should return old dark order book when overwriting', async () => {
      await prepareDeposit(user);
      let encryptedOrdersV1 = await encryptOrders([
        { kind: 'ask', volume: 1000000, price: 42.5 },
        {
          kind: 'bid',
          volume: 12300,
      ...
      let resp2 = await auction.manageDarkOrderBooks(
        [
          [
            ...
            [
              await encryptOrders([
                { kind: 'ask', volume: 1120000, price: 39.5 },
                {
                  kind: 'bid',
                  volume: 10300,
                  price: 9.2,
    ...
    test('should process dark orders', async () => {
      ...
      expect(
        (
          await auction.auction_query([quoteLedgerPrincipal, ledger1Principal], {
            ...auctionQueryEmpty,
            dark_order_books: [true],
          })
        ).dark_order_books,
      ...
      auction.setIdentity(user);
      let state = await auction.auction_query([quoteLedgerPrincipal, ledger1Principal], {
        ...auctionQueryEmpty,
        credits: [true],
        dark_order_books: [true],
      });
    ...
    test('should process dark order partially when out of funds', async () => {
      ...
      auction.setIdentity(seller);
      state = await auction.auction_query([quoteLedgerPrincipal, ledger1Principal], {
        ...auctionQueryEmpty,
        credits: [true],
        asks: [true],
      });
    ...
    test('should remove unfulfilled dark order after auction ran', async () => {
      ...
      let state0 = await auction.auction_query([quoteLedgerPrincipal, ledger1Principal], {
        ...auctionQueryEmpty,
        dark_order_books: [true],
        credits: [true],
        bids: [true],
      ...
      let state1 = await auction.auction_query([quoteLedgerPrincipal, ledger1Principal], {
        ...auctionQueryEmpty,
        dark_order_books: [true],
        credits: [true],
        bids: [true],
