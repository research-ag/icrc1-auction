...
module CircularBuffer {
  public class CircularBuffer<T>(capacity : Nat) {
    ...
    var array : [var ?T] = VarArray.repeat(null, capacity);
    var last = 0;
    var pushes = 0;
  ...
  func bytesToPages(n : Nat) : Nat64 {
    ((n + (PAGE_SIZE - 1) : Nat) / PAGE_SIZE).toNat64();
  };
  ...
  public class CircularBufferStable<T>(
    ...
    func pointerOffset(i : Index) : Nat64 {
      (i % (capacity + 1) * POINTER_SIZE).toNat64();
    };

    func storePointer(i : Index, addr : Address) {
      Region.storeNat32(state().index, pointerOffset(i), addr.toNat32());
    };
    ...
    func storeData(addr : Address, blob : Blob) {
      Region.storeBlob(state().data, addr.toNat64(), blob);
    };

    func loadData(addr : Address, len : Nat) : Blob {
      Region.loadBlob(state().data, addr.toNat64(), len);
    };
    ...
    func loadInterval(from : Address, to : Address) : T {
      let blob = if (from < to) {
        ...
        Array.tabulate<Nat8>(sz + to, func(i) = if (i < sz) unwrap(next1()) else unwrap(next2())).toBlob();
      } else {
        "" : Blob;
      };
      deserialize(blob);
    ...
    func push_(item : T, force : Bool) : Bool {
      ...
      if (item_length > 0) {
        ...
        if (end_data + item_length <= length) {
          ...
          let part1 = Array.tabulate<Nat8>(sz, func(i) = unwrap(next())).toBlob();
          let part2 = Array.tabulate<Nat8>(item_length - sz, func(i) = unwrap(next())).toBlob();
