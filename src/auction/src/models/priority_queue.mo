...
module {
  ...
  public func insert<T>(self : PriorityQueue<T>, item : T, comparePriority : (T, T) -> Order.Order) : (PriorityQueue<T>, Nat) {
    switch self {
      case null (?(item, null), 0);
      case (?(h, tail)) {
        switch (comparePriority(h, item)) {
          case (#less) (?(item, self), 0);
  ...
  public func findOneAndDelete<T>(self : PriorityQueue<T>, f : T -> Bool) : (PriorityQueue<T>, ?T) {
    switch self {
      case null { (null, null) };
      case (?(h, t)) {
        if (f(h)) {
