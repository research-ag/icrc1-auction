...
module {
  ...
  public func init() : Bench.Bench {
    ...
    bench.name("Auction processing (high volumes)");
    bench.description("Process a trading pair which has N placed orders and high token precision, e.g. deal volumes have 18+ digits");
    ...
    let auctions : [Auction.Auction] = Array.tabulate<Auction.Auction>(
      ...
      func(i) {
        ...
        assert a.assets.getAsset(1).bids.amount == nOrders / 2;
        assert a.assets.getAsset(1).asks.amount == nOrders / 2;
        a;
      },
    );
    ...
    bench.runner(
      func(row, col) {
        ...
        // assert auction.assets.getAsset(1).bids.amount + nBids == nOrders / 2;
        // assert auction.assets.getAsset(1).asks.amount + nAsks == nOrders / 2;
      }
    );
