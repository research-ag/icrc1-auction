...
do {
  ...
  let ft = 123;
  let res = auction.placeOrder(user, #bid, ft, #delayed, 2_000, 100_000, null, runtime);
  switch (res) {
...
do {
  ...
  let ft = 0;
  switch (auction.placeOrder(user, #bid, ft, #delayed, 2_000, 100_000, null, runtime)) {
    case (#err(#UnknownAsset)) ();
    case (_) assert false;
  };
  assert auction.getOrders(user, #bid, ?ft).size() == 0;
...
do {
  ...
  switch (auction.placeOrder(user, #bid, ft, #delayed, 2_000, 1_000_000, null, runtime)) {
    case (#err(#NoCredit)) ();
    case (_) assert false;
  };
  assert auction.getOrders(user, #bid, ?ft).size() == 0;
...
do {
  ...
  let ft = createFt(auction);
  switch (auction.placeOrder(user, #bid, ft, #delayed, 20, 100, null, runtime)) {
...
do {
  ...
  let ft = createFt(auction);
  switch (auction.placeOrder(user, #bid, ft, #delayed, 2_000, 1_000, null, runtime)) {
    case (#ok _) ();
    case (_) assert false;
  };
...
do {
  ...
  let ft = createFt(auction);
  ignore auction.placeOrder(user, #bid, ft, #delayed, 2_000, 15_000, null, runtime);
...
do {
  ...
  let ft = createFt(auction);
  switch (auction.placeOrder(user, #bid, ft, #delayed, 1_000, 400_000, null, runtime)) {
    case (#ok _) ();
    case (_) assert false;
  };
  switch (auction.placeOrder(user, #bid, ft, #delayed, 1_000, 400_000, null, runtime)) {
...
do {
  ...
  ignore auction.placeOrder(user, #bid, ft, #delayed, 2_000, 100_000, null, runtime);
  assert auction.getCredit(user, 0).available == 300_000_000;
  switch (auction.placeOrder(user, #bid, ft, #delayed, 2_000, 150_000, null, runtime)) {
    case (#ok _) ();
    case (_) assert false;
  };
...
do {
  ...
  assert auction.getCredit(user, 0).available == 500_000_000;
  let orderId = switch (auction.placeOrder(user, #bid, ft, #delayed, 2_000, 100_000, null, runtime)) {
    case (#ok(id, _)) id;
...
do {
  ...
  let orderId = switch (auction.placeOrder(user, #bid, ft, #delayed, 2_000, 125_000, null, runtime)) {
    case (#ok(id, _)) id;
    case (_) { assert false; 0 };
  };
  ...
  let newOrderId = switch (auction.replaceOrder(user, #bid, orderId, 2_000, 250_000, null, runtime)) {
    case (#ok(id, _)) id;
    case (_) { assert false; 0 };
  };
  assert auction.getCredit(user, 0).available == 0;
  ...
  switch (auction.replaceOrder(user, #bid, newOrderId, 2_000, 60_000, null, runtime)) {
    case (#ok _) {};
    case (_) assert false;
  };
...
do {
  ...
  switch (auction.replaceOrder(user, #bid, orderId, 2_000_000, 250_000_000, null, runtime)) {
    case (#err(#NoCredit)) ();
    case (_) assert false;
  };
  assert auction.getCredit(user, 0).available == 250_000_000;
...
do {
  ...
  switch (auction.placeOrder(user, #bid, ft, #delayed, 2_000, 15_000, null, runtime)) {
    case (#ok _) {};
    case (_) assert false;
  };
  assert auction.getOrders(user, #bid, ?ft).size() == 1;
...
do {
  ...
  ignore auction.placeOrder(user, #bid, ft1, #delayed, 1_500, 100_000, null, runtime);
  ignore auction.placeOrder(user, #bid, ft2, #delayed, 1_500, 100_000, null, runtime);
  ignore auction.placeOrder(user2, #bid, ft1, #delayed, 1_500, 100_000, null, runtime);
  ignore auction.placeOrder(user2, #bid, ft2, #delayed, 1_500, 100_000, null, runtime);

  let seller = Principal.fromText("ocqy6-3dphi-xgf54-vkr2e-lk4oz-3exc6-446gr-5e72g-bsdfo-4nzrm-hqe");
  ignore auction.appendCredit(seller, ft1, 500_000_000);
  ignore auction.appendCredit(seller, ft2, 500_000_000);
  ignore auction.placeOrder(seller, #ask, ft1, #delayed, 200_000_000, 100_000, null, runtime);
  ignore auction.placeOrder(seller, #ask, ft2, #delayed, 200_000_000, 100_000, null, runtime);
...
do {
  ...
  let seller = Principal.fromText("ocqy6-3dphi-xgf54-vkr2e-lk4oz-3exc6-446gr-5e72g-bsdfo-4nzrm-hqe");
  ignore auction.appendCredit(seller, ft, 500_000_000);
  ignore auction.placeOrder(seller, #ask, ft, #delayed, 2_000, 100_000, null, runtime);
  ...
  let user2 = Principal.fromText("tbsil-wffo6-dnxyb-b27v7-c5ghk-jsiqs-gsok7-bmtyu-w7u3b-el75k-iae");
  ignore auction.appendCredit(user2, 0, 500_000_000);
  ignore auction.placeOrder(user2, #bid, ft, #delayed, 999, 100_000, null, runtime);
...
do {
  ...
  let seller = Principal.fromText("ocqy6-3dphi-xgf54-vkr2e-lk4oz-3exc6-446gr-5e72g-bsdfo-4nzrm-hqe");
  ignore auction.appendCredit(seller, ft, 500_000_000);
  ignore auction.placeOrder(seller, #ask, ft, #delayed, 3_500, 100_000, null, runtime);
  assert auction.getOrders(seller, #ask, ?ft).size() == 1;

  ignore auction.appendCredit(user, 0, 500_000_000);
  ignore auction.placeOrder(user, #bid, ft, #delayed, 1_500, 200_000, null, runtime);
  assert auction.getOrders(user, #bid, ?ft).size() == 1;

  let user2 = Principal.fromText("tbsil-wffo6-dnxyb-b27v7-c5ghk-jsiqs-gsok7-bmtyu-w7u3b-el75k-iae");
  ignore auction.appendCredit(user2, 0, 500_000_000);
  ignore auction.placeOrder(user2, #bid, ft, #delayed, 1_500, 150_000, null, runtime);
  assert auction.getOrders(user2, #bid, ?ft).size() == 1;

  let user3 = Principal.fromText("3ekl2-xv73q-5v4oc-u3edq-dykz6-ps2k6-jxjiu-34myc-zc6rg-ucex3-4qe");
  ignore auction.appendCredit(user3, 0, 500_000_000);
  ignore auction.placeOrder(user3, #bid, ft, #delayed, 1_500, 100_000, null, runtime);
  assert auction.getOrders(user3, #bid, ?ft).size() == 1;
...
do {
  ...
  let (auction, runtime, _) = init(0, 3, 5);
  let ft = createFt(auction);
  let seller = Principal.fromText("ocqy6-3dphi-xgf54-vkr2e-lk4oz-3exc6-446gr-5e72g-bsdfo-4nzrm-hqe");
  ignore auction.appendCredit(seller, ft, 500_000_000);
  ...
  ignore auction.placeOrder(mediumBidder, #bid, ft, #delayed, 1_500, 20_000, null, runtime);
  ...
  ignore auction.placeOrder(highBidder, #bid, ft, #delayed, 1_500, 50_000, null, runtime);
  ...
  ignore auction.placeOrder(lowBidder, #bid, ft, #delayed, 1_500, 5_000, null, runtime);
  ...
  ignore auction.placeOrder(newBidder, #bid, ft, #delayed, 1_500, 20_000, null, runtime);
